...
description: Enterprise-grade, evidence-driven SEO auditing of static code repositories. Evaluates HTML, Markdown, JS, CSS, JSON, and config files across 24 audit pillars â€” including competitor analysis, semantic SEO, video/YouTube SEO, voice search optimization, IndexNow protocol, and crawl orchestration â€” to surface production-ready fixes. Generates comprehensive seo_audit_report.md with prioritized remediation and CSV export.
...
**Context-aware criteria.** A portfolio site does not need Product schema. A local business site without NAP consistency is Critical. An e-commerce site without breadcrumb schema is Medium. Adjust the bar for each project type â€” do not apply the same defaults to every audit.

**Crawl the graph, not the list.** When auditing a single file, trace every linked dependency: stylesheets, scripts, images, iframes, and anchor hrefs. A broken image reference in `index.html` is not an On-Page issue â€” it is a Performance (CLS) and Image SEO issue that cascades. Follow the dependency chain.
...
**First pass â€” workspace exploration & discovery.** Before analyzing individual files, index the structural footprint of the workspace. Map the file directory tree â€” enumerate every physical directory and file path, noting logical groupings (`/page`, `/page/projects`, `/blog`, `/assets`, `/sitemap.html`, etc.). Scan for metadata configuration files (`package.json`, `config.js`, front-matter defaults in Markdown files, `.env`, build configs) to understand routing rules, build system, and site metadata. Determine how routing and layout injection work â€” is this pure static HTML, a static site generator (Astro, Hugo, 11ty), a client-side rendered SPA, or an SSR framework? This determines which pillars apply and how indexability is evaluated.
...
**Second pass â€” analyze against pillars.** Evaluate each file against the relevant pillars from the list below. For each pillar, determine which checks apply given the project type. If the project is a static HTML site, skip JavaScript Framework SEO entirely. If the project has no blog, skip Blog & Content SEO. Be disciplined about suppressing irrelevant pillars.

**Third pass â€” prioritize.** Sort all findings by severity, then by estimated traffic impact within each severity level. Produce the Priority Fix Matrix. The first item in the matrix should be the single most impactful fix the user can make. If you have more than 20 items in the matrix, you are listing too many â€” consolidate or deprioritize.

**Fourth pass â€” write the report.** Generate `seo_audit_report.md` following the Output Blueprint below. Use professional, direct language. Never hedge ("this might affect rankings" â€” either it does or it doesn't). Every severity label, every root cause, every fix block must be defensible.

**Fifth pass â€” critique the report before delivering.** Review your own output as if you were a competitor's SEO lead. For every finding, ask: is the fix correct? Is the severity justified? Is there a simpler approach? Would I ship this report to a paying client? If the answer to any of these is no, revise before presenting. This is your quality gate.

## Restraint and self-critique

The most common failure pattern in AI-generated SEO audits is false positives â€” flagging issues that aren't real problems in context. Before including a finding, ask yourself: would Google actually penalize or rank-lower this site for this specific issue? If the answer is "probably not at this site's scale/type," demote or drop the finding.

Do not flag the same issue in every file as separate findings. If every page lacks a `lang` attribute, that is **one** finding â€” list the affected files in the location field. Duplicate entries waste the user's attention and dilute real priorities.

Beware of checklist-driven auditing where you mechanically tick boxes without considering whether the check matters. An `<h1>` that matches the `<title>` is not inherently problematic â€” it only matters when combined with thin content or keyword cannibalization. Always ask "so what?" before writing a finding.
...
- **Keyword Cannibalization (Cross-Page):** Scan all pages for multiple files targeting the same primary keyword cluster. Focus on title tag and H1 overlap across pages â€” if two pages compete for the same query, neither ranks fully. (Content-level cannibalization based on semantic overlap is in Pillar 7.)
...
- **SERP Snippet Quality:** Assess how titles and descriptions render in search results â€” check for ellipsis truncation, brand inclusion, call-to-action language, and competitive differentiation. Flag descriptions that duplicate the title or lack a unique value proposition.
- **Structured Content Layout:** Check for scannable formatting â€” bullet lists, bolding of key phrases, short paragraphs (<5 sentences), blockquotes for testimonials, callout boxes. Flag walls of text without visual hierarchy.
...
- **Keyword Placement Scoring:** For each page, evaluate whether the primary target keyword appears in the five highest-signal placement zones:
  1. **Title tag** (highest weight â€” must contain primary keyword)
...
- **Robots.txt Deep Audit:** Validate `User-agent`, `Disallow`, `Allow`, `Sitemap`, `Crawl-delay`. Flag overly broad `Disallow: /` on production sites. Test critical paths against robots.txt rules. Check for crawl-delay directives that may slow Googlebot. (Canonical definition in Pillar 9 â€” this check is technical-level; Pillar 9 covers full robots.txt + sitemap ecosystem.)
- **JavaScript SEO:** Detect client-side rendered content. Assess crawlability â€” flag content that requires JS execution to be indexed. Recommend SSR, SSG, or static fallback strategies.
...
- **Broken Internal Links:** Verify all internal `href` destinations resolve to an existing file in the workspace. Provide a complete broken link inventory. (Canonical definition in Pillar 8 â€” this check is technical-level; Pillar 8 covers link equity and navigation impact.)
...
- **Mobile vs Desktop Performance Splits:** Evaluate performance metrics separately for mobile and desktop viewports. Mobile LCP, CLS, and INP thresholds are stricter â€” flag issues that are mobile-specific (viewport-relative image sizes, touch event handling, reduced network conditions).
...
- **URL Slug Quality Evaluation:** Beyond format, evaluate slug quality:
  ...
  - **Stop word bloat:** Flag slugs with excessive stop words (e.g., `/the-best-of-the-most-amazing-seo-tool` â†’ `/best-seo-tool`).
  ...
  - **Slug length:** Flag slugs exceeding 5 words â€” long slugs dilute keyword focus and reduce shareability.
...
- **Silo Architecture:** Evaluate topical clustering â€” do pages about related topics link to each other? Is there a pillar page that links to cluster content?
- **Orphan Pages:** Check for HTML files not linked from any other page in the workspace. (Canonical definition in Pillar 8 â€” this check is architecture-level; Pillar 8 covers link equity impact.)
- **Redirect Chains (Static Analysis):** Flag any `http-equiv="refresh"` based redirects. Flag `meta refresh` redirects with delay >0.

### 5. Mobile SEO
- **Viewport Configuration:** Enforce `width=device-width, initial-scale=1` â€” no `user-scalable=no` or `maximum-scale=1` (accessibility & ranking violation).
...
- **Search Intent Mapping:** Evaluate text content against primary target keywords â€” is the content informational, navigational, transactional, or commercial? Flag intent mismatch. Go beyond basic intent: map each page to granular query types (comparison "X vs Y", best-of "best X", how-to "how to X", definitional "what is X", transactional "buy X").
...
- **Entity Salience Analysis:** Evaluate whether the most important entities for the topic receive proportional content weight. The primary entity should appear early, be defined clearly, and have the highest frequency-weighted prominence. Flag pages where secondary entities overshadow the primary topic. This is an LLM judgment call â€” use semantic reasoning to assess whether the content's focus matches the intended primary topic.
...
- **IndexNow Protocol Setup:** Check for IndexNow implementation â€” an open protocol that allows sites to notify search engines instantly about content changes, supporting Bing, Yandex, Seznam, and Naver. If absent, recommend setup:
  - Generate a unique API key (a UUID or random hex string).
  - Host the key file at `https://www.example.com/<key>.txt` containing only the key value â€” this file must be UTF-8, served at the root level, and publicly accessible.
...
- **Author Transparency Depth:** Evaluate author byline completeness â€” full name (not initials), professional photo, credential disclosure, social media links, publication history, and topical expertise indicators. Flag anonymous or pseudonymous content on authority-dependent pages.
...
- **Automated Contact Pathway Detection:** For the primary contact method, verify the detection priority chain:
  1. **Dedicated Contact Page** (highest trust â€” verifiable address, phone, form)
  2. **About Page with Contact Details** (secondary â€” email, phone in bio section)
  3. **Footer Contact Info** (minimal â€” email, phone, address in footer)
  4. **Social Media Links Only** (lowest â€” no direct contact method)
  Flag sites where the only contact method is social media links (no verifiable email, phone, or address). For local businesses, this is a Critical finding â€” Google requires a verifiable contact method for Local Business schema eligibility.
...
- **Content Credibility Scoring:** Evaluate factual accuracy signals â€” date-appropriate references, cited sources recency, claims-to-evidence ratio, and absence of outdated information. Flag content with unsupported claims, stale statistics (older than 2 years for fast-moving topics), or references to superseded research. This is an LLM judgment call â€” use semantic reasoning to assess whether claims are supported by the evidence presented.
...
- **Site-Wide Trust Signals (Unified Audit):** Consolidate trust signal verification in one pass â€” SSL certificate enforcement, privacy policy page, terms of service page, accessibility statement, cookie consent implementation, data processing disclosure, returns/refund policy (e-commerce), shipping policy (e-commerce). Flag any missing legal or trust pages.
...
- **Rich Result Opportunity Map:** Based on the site's content type, provide a checklist of eligible rich result types and the exact schema required for each:
  - **Article Rich Results:** `Article`, `NewsArticle`, `BlogPosting` â€” requires `headline`, `image`, `datePublished`, `author`, `publisher` (with `logo`)
  - **Product Rich Results:** `Product` â€” requires `name`, `image`, `offers.price`, `offers.priceCurrency`, `offers.availability`
  - **Review Snippet:** `Review` â€” requires `itemReviewed`, `reviewRating.ratingValue`, `author`
  - **Recipe Rich Results:** `Recipe` â€” requires `name`, `image`, `recipeIngredient`, `recipeInstructions`
  - **FAQ Rich Results:** `FAQPage` â€” requires `mainEntity` with `Question` and `AcceptedAnswer`
  - **How-To Rich Results:** `HowTo` â€” requires `name`, `step` with `Step` and `text`
  - **Local Business:** `LocalBusiness` â€” requires `name`, `address`, `telephone`
  - **BreadcrumbList:** `BreadcrumbList` â€” requires `itemListElement` with `ListItem`
  - **Sitelinks Search Box:** `WebSite` with `potentialAction` and `SearchAction` â€” requires `target` and `query-input`
  - **Video Rich Results:** `VideoObject` â€” requires `name`, `description`, `thumbnailUrl`, `uploadDate`
  - **Event Rich Results:** `Event` â€” requires `name`, `startDate`, `location` or `virtualLocation`
  - **Job Posting:** `JobPosting` â€” requires `title`, `description`, `datePosted`, `hiringOrganization`
  - **Course:** `Course` â€” requires `name`, `description`, `provider`
  - **Book:** `Book` â€” requires `name`, `author`, `workExample` with `isbn`
  - **Software App:** `SoftwareApplication` â€” requires `name`, `operatingSystem`, `applicationCategory`
  - **Organization:** `Organization` â€” requires `name`, `logo` (112x112px min), `url`, `contactPoint`, `sameAs` for social profiles
  - **Service:** `Service` â€” requires `name`, `description`, `provider`, `areaServed`, `serviceType`
  - **Person:** `Person` â€” requires `name`, `url`, `sameAs` (social profiles), `jobTitle`, `knowsAbout` (topical expertise), `alumniOf`
...
- **llms.txt Check:** Verify the site publishes a `llms.txt` file at the root â€” a machine-readable summary of the site for LLM discoverability. Flag missing `llms.txt`. Recommend including: site name, description, key pages list, preferred citation format, and content licensing info.
...
- **Ad Script Impact:** Flag ad scripts that:
  - Cause layout shift (no reserved ad slots) â€” Google will penalize CLS >0.1
  - Load before content â€” ads should never delay content rendering
...
- **Font Loading Optimization:** Verify `font-display: swap` in `@font-face` declarations. Flag self-hosted fonts without WOFF2 format. Flag missing `preconnect` for Google Fonts. (Performance-level checks are in Pillar 3 â€” this check focuses on third-party script impact.)
...
**Use the active voice for fixes.** "Add `<meta name="viewport">` to the `<head>` of `index.html:5`" â€” not "The viewport meta tag should be considered for addition."
...
**End with the user's next action.** The final line of the report (before Resources & References) should be the single most important thing the user should do right now. Not a summary â€” a command: "Fix the missing canonical on `index.html` first, then run `npm run build && npx html-validate dist/` to verify."

**Report detail writing rules:**
- **Never write vague findings.** "SEO could be improved" is not a finding. "The `<title>` tag on `/about.html` is 12 characters â€” 40 characters below the minimum for effective SERP display" is a finding.
...
- **Include the "so what?" for every finding.** After stating the problem, always answer: "Why does this matter to the business?" A missing alt text attribute is not inherently a problem â€” it becomes a problem when it affects image search rankings, accessibility compliance, or rich result eligibility.
- **Provide file-level specificity.** Never say "some pages" or "multiple files." List every affected file path. If there are 50 files with the same issue, list all 50 in the Location field â€” do not summarize to "various files."
...
- **Group related findings.** If three pages all have the same missing canonical issue, present it as one finding with three file locations â€” not three separate findings. Consolidate to keep the report actionable.
...
```
@SKILL.md Run a [Next.js/Nuxt/Astro/React-SPA] SEO audit. Focus on framework-specific patterns â€” SSR configuration, meta management, image optimization, routing strategy, and hydration approach.
```
...
```
@SKILL.md Run a Social SEO quality audit. Go beyond presence checks â€” evaluate og:image dimensions and file size, og:title vs title divergence, twitter:card type appropriateness, og:description quality, and cross-platform consistency. Flag field-level quality issues that reduce social share CTR.
```
...
**Mandatory finding format (Evidence / Impact / Fix):** Every finding in the report must follow this three-part structure â€” no exceptions:
...
**Report customization note:** Headings can use emoji markers (ðŸŽ¯, ðŸ”, ðŸ› ï¸, ðŸ’») for stakeholder readability if preferred. The report should include an attribution line: "Generated by [Team Name] â€” [URL]" at the top or bottom. Findings can alternatively be grouped by fix type (schema fixes, metadata fixes, content fixes, performance fixes) rather than by pillar, if the engineering team prefers workflow-based grouping.

```markdown
# ðŸ“Š STRATEGIC SEO AUDIT REPORT
**Generated by:** [Team Name â€” URL]  
**Generated on:** [Date]  
...
[Foundational improvements â€” site architecture redesign, content hub restructuring, migration planning â€” that yield compounding returns over 6-12 months.]
...
- **Issues Found:**
  - **Issue N:** [title]
    ...
    - **Fix:**
      ```html
      <!-- Production-ready replacement â€” HTML, CSS, JS, JSON-LD, or config as appropriate -->
      ```
    ...
    - **Verification:** [how to confirm the fix is correct â€” specific tool, command, or visual check]
