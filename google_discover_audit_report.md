...
- **Site:** F9XR Articles - `https://f9xr.org/articles`
...
- `max-image-preview:large` in robots meta â€” required for large Discover thumbnails.
- All post heroes are WebP at 1200Ã—630 or 1536Ã—1024, well above the 1200px minimum.
...
### 2. Image requirements â€” PASS

| Post hero | Format | Dimensions | >= 1200px? |
|-----------|--------|-----------|------------|
| zero-click-search-google-business-profile-homepage.webp | WebP | 1200Ã—630 | Yes |
| how-ai-picks-local-businesses-2026.webp | WebP | 1200Ã—630 | Yes |
| rank-1st-on-google-maps.webp | WebP | 1536Ã—1024 | Yes |
| f9xr-team-solutions.webp | WebP | 1600Ã—893 | Yes |
| f9xr-dic.webp | WebP | â€” | Yes |

- `max-image-preview:large` present â†’ large Discover thumbnails allowed.
- Default OG image `og-image.webp` is 1200Ã—630.
...
### 4. Page experience / mobile â€” PASS (recently hardened)
...
### 5. Freshness â€” HIGH RISK (issue High 1)
...
### 6. Structured data â€” PARTIAL (issue Medium 1)
...
### 7. E-E-A-T and authorship â€” PASS
...
### 8. Content quality and cadence â€” PASS

- 19 posts in ~3 weeks, evergreen and how-to heavy, no thin content.
- Topically focused (local SEO, AI search, AEO/GEO, digital growth) â€” matches an audience Discover already serves.
- Each post has keyword target, description â‰¤160, and 10 tags.

### 9. Feed / sitemap â€” PASS (with note)
...
**High 1 â€” Missing `dateModified` on the three newest posts (08-16).**
...
**High 2 â€” Discover requires Search visibility first.**
...
**Medium 1 â€” No explicit `Article` JSON-LD.**
...
**Medium 2 â€” `dateModified` not auto-synced to a template signal.**
...
**Medium 3 â€” No Search Console verification documented.**
Verify ownership of `f9xr.org` in Search Console and confirm the `/articles` sitemap is submitted and free of errors; without this, Discover performance cannot be measured or diagnosed.

### Low

**Low 1 â€” Hero images have no explicit `image` in frontmatter-level structured data beyond Open Graph.** Already covered by OG tags; optional to mirror into JSON-LD.

**Low 2 â€” `max-video-preview:-1` is set, which is fine for SEO, but embedded YouTube videos rely on the standard `web-share` allowlist; verify iframes still autoplay-thumbnail correctly on mobile (low risk).**

**Low 3 â€” No social proof / engagement surface.** Discover favors content that earns engagement once surfaced; sharing links and the new GitHub open-source badge are good, but consider encouraging comments/CTAs on post pages over time.
...
*Generated by F9XR SEO Codebase Audit â€” Google Discover eligibility pillar.*
